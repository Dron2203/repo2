...
def connect_to_db(host, port, db_name, user, password):
    connection = pymysql.connect(host=host, user=user, port=port, password=password, db=db_name)
    cursor = connection.cursor()
    return cursor, connection

if __name__ == '__main__':
    cursor, connection = connect_to_db('192.168.1.87',3306,'test','student','123456')
